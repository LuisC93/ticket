// auth.js — Sistema de usuarios y autenticación Monitoreo C.E

// ── CONSTANTES ──
var AUTH_USERS_KEY = 'mce_users';
var AUTH_SESSION_KEY = 'mce_session';

var AUTH_PERMS = {
  'Admin':             { crear: true,  cerrar: true,  editar: true,  verTodos: true,  usuarios: true,  config: true  },
  'Monitor':           { crear: true,  cerrar: false, editar: false, verTodos: false, usuarios: false, config: false },
  'Monitor/Técnico':   { crear: true,  cerrar: true,  editar: false, verTodos: false, usuarios: false, config: false },
  'Técnico':           { crear: false, cerrar: true,  editar: false, verTodos: false, usuarios: false, config: false }
};

var AUTH_DEFAULT_USERS = [
  { id: 1, nombre: 'Administrador', username: 'admin', password: 'admin123', rol: 'Admin' }
];

// ── ESTADO ──
var currentUser = null;

// ── HELPERS ──
function authGetUsers() {
  var s = localStorage.getItem(AUTH_USERS_KEY);
  return s ? JSON.parse(s) : JSON.parse(JSON.stringify(AUTH_DEFAULT_USERS));
}
function authSaveUsers(u) { localStorage.setItem(AUTH_USERS_KEY, JSON.stringify(u)); }
function authCan(perm) { return currentUser && AUTH_PERMS[currentUser.rol] && AUTH_PERMS[currentUser.rol][perm]; }

// ── INICIALIZAR ──
function authInit() {
  // Inyectar pantalla login en el DOM
  injectLoginScreen();
  injectUserModal();
  injectTopbarUser();

  // Revisar sesión activa
  var s = sessionStorage.getItem(AUTH_SESSION_KEY);
  if (s) {
    try {
      var u = JSON.parse(s);
      // Verificar que el usuario aún existe
      var users = authGetUsers();
      var found = users.find(function(x) { return x.id === u.id && x.username === u.username; });
      if (found) {
        currentUser = found;
        showMainApp();
        return;
      }
    } catch(e) {}
  }
  showLoginScreen();
}

// ── LOGIN / LOGOUT ──
function doLogin() {
  var u = document.getElementById('auth-username').value.trim();
  var p = document.getElementById('auth-password').value;
  var err = document.getElementById('auth-err');
  if (!u || !p) { err.textContent = 'Completa usuario y contraseña.'; return; }
  var users = authGetUsers();
  var found = users.find(function(x) { return x.username === u && x.password === p; });
  if (!found) { err.textContent = 'Usuario o contraseña incorrectos.'; shakeLoginCard(); return; }
  err.textContent = '';
  currentUser = found;
  sessionStorage.setItem(AUTH_SESSION_KEY, JSON.stringify(found));
  showMainApp();
}

function doLogout() {
  sessionStorage.removeItem(AUTH_SESSION_KEY);
  currentUser = null;
  showLoginScreen();
}

function shakeLoginCard() {
  var card = document.getElementById('auth-card');
  card.style.animation = 'none';
  setTimeout(function() { card.style.animation = 'authShake .4s ease'; }, 10);
}

// ── MOSTRAR / OCULTAR ──
function showLoginScreen() {
  document.getElementById('auth-screen').style.display = 'flex';
  var app = document.querySelector('.app');
  var sidebar = document.querySelector('.sidebar');
  if (app) app.style.display = 'none';
  if (sidebar) sidebar.style.display = 'none';
  document.getElementById('auth-username').value = '';
  document.getElementById('auth-password').value = '';
  document.getElementById('auth-err').textContent = '';
  setTimeout(function() { document.getElementById('auth-username').focus(); }, 100);
}

function showMainApp() {
  document.getElementById('auth-screen').style.display = 'none';
  var app = document.querySelector('.app');
  var sidebar = document.querySelector('.sidebar');
  if (app) app.style.display = 'flex';
  if (sidebar) sidebar.style.display = 'flex';
  applyRoleUI();
  updateTopbarUser();
}

// ── APLICAR PERMISOS EN UI ──
function applyRoleUI() {
  if (!currentUser) return;
  var perms = AUTH_PERMS[currentUser.rol];

  // Sidebar: ocultar tabs según rol
  var btnNuevo = document.querySelector('.sidebar-item[onclick*="nuevo"]');
  var btnTecnico = document.querySelector('.sidebar-item[onclick*="tecnico"]');

  if (btnNuevo) btnNuevo.style.display = perms.crear ? 'flex' : 'none';
  if (btnTecnico) btnTecnico.style.display = (perms.cerrar || authCan('verTodos')) ? 'flex' : 'none';

  // Botón configurar (solo admin)
  var btnCfg = document.querySelector('.btn-report');
  if (btnCfg) btnCfg.style.display = perms.config ? 'flex' : 'none';

  // Si no puede crear, redirigir a monitor o técnico según rol
  if (!perms.crear) {
    var btnMon = document.querySelector('.sidebar-item[onclick*="monitor"]');
    if (btnMon) { btnMon.click(); }
  }

  // Panel usuarios en sidebar (solo admin)
  var btnUsers = document.getElementById('sidebar-users-btn');
  if (btnUsers) btnUsers.style.display = perms.usuarios ? 'flex' : 'none';
}

// ── TOPBAR: mostrar usuario activo ──
function injectTopbarUser() {
  var topRight = document.querySelector('.topbar-right');
  if (!topRight) return;

  var div = document.createElement('div');
  div.id = 'auth-topbar-user';
  div.style.cssText = 'display:flex;align-items:center;gap:8px';
  div.innerHTML =
    '<div id="auth-user-pill" style="display:flex;align-items:center;gap:8px;padding:5px 12px 5px 7px;border-radius:20px;border:1.5px solid var(--border);background:var(--white);cursor:pointer" onclick="toggleUserMenu()">' +
      '<div id="auth-avatar" style="width:26px;height:26px;border-radius:50%;background:#2563eb;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#fff">A</div>' +
      '<span id="auth-user-name" style="font-size:13px;font-weight:600;color:var(--text)">Admin</span>' +
      '<span id="auth-role-badge" style="font-size:10px;padding:2px 8px;border-radius:10px;font-weight:600;background:#eff6ff;color:#2563eb">Admin</span>' +
    '</div>' +
    '<div id="auth-user-menu" style="display:none;position:absolute;top:52px;right:28px;background:var(--white);border:1.5px solid var(--border);border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.1);z-index:500;min-width:220px;overflow:hidden">' +
      '<div id="auth-menu-info" style="padding:14px 16px;border-bottom:1px solid var(--border)">' +
        '<div style="font-size:13px;font-weight:700;color:var(--text)" id="auth-menu-nombre"></div>' +
        '<div style="font-size:11px;color:var(--text3)" id="auth-menu-username"></div>' +
      '</div>' +
      '<button onclick="openUsersPanel()" id="auth-menu-usuarios" style="width:100%;text-align:left;padding:10px 16px;border:none;background:none;font-size:13px;color:var(--text);cursor:pointer;display:flex;align-items:center;gap:8px;font-family:var(--sans)">' +
        '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>' +
        'Gestionar usuarios' +
      '</button>' +
      '<button onclick="doLogout()" style="width:100%;text-align:left;padding:10px 16px;border:none;background:none;font-size:13px;color:var(--danger);cursor:pointer;display:flex;align-items:center;gap:8px;font-family:var(--sans);border-top:1px solid var(--border)">' +
        '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>' +
        'Cerrar sesión' +
      '</button>' +
    '</div>';
  topRight.insertBefore(div, topRight.firstChild);

  // cerrar menu al click fuera
  document.addEventListener('click', function(e) {
    var menu = document.getElementById('auth-user-menu');
    var pill = document.getElementById('auth-user-pill');
    if (menu && pill && !pill.contains(e.target) && !menu.contains(e.target)) {
      menu.style.display = 'none';
    }
  });
}

function updateTopbarUser() {
  if (!currentUser) return;
  var rolColors = {
    'Admin':           { bg: '#eff6ff', color: '#2563eb', av: '#2563eb' },
    'Monitor':         { bg: '#f0fdf4', color: '#16a34a', av: '#16a34a' },
    'Monitor/Técnico': { bg: '#f5f3ff', color: '#7c3aed', av: '#7c3aed' },
    'Técnico':         { bg: '#fffbeb', color: '#d97706', av: '#d97706' }
  };
  var rc = rolColors[currentUser.rol] || rolColors['Monitor'];
  var initials = currentUser.nombre.split(' ').map(function(w) { return w[0]; }).slice(0,2).join('').toUpperCase();

  var av = document.getElementById('auth-avatar');
  if (av) { av.textContent = initials; av.style.background = rc.av; }

  var nm = document.getElementById('auth-user-name');
  if (nm) nm.textContent = currentUser.nombre.split(' ')[0];

  var rb = document.getElementById('auth-role-badge');
  if (rb) { rb.textContent = currentUser.rol; rb.style.background = rc.bg; rb.style.color = rc.color; }

  var mn = document.getElementById('auth-menu-nombre');
  if (mn) mn.textContent = currentUser.nombre;
  var mu = document.getElementById('auth-menu-username');
  if (mu) mu.textContent = '@' + currentUser.username;

  // Ocultar "Gestionar usuarios" si no es admin
  var muBtn = document.getElementById('auth-menu-usuarios');
  if (muBtn) muBtn.style.display = authCan('usuarios') ? 'flex' : 'none';
}

function toggleUserMenu() {
  var menu = document.getElementById('auth-user-menu');
  if (menu) menu.style.display = menu.style.display === 'none' ? 'block' : 'none';
}

// ── PANEL USUARIOS ──
function openUsersPanel() {
  var menu = document.getElementById('auth-user-menu');
  if (menu) menu.style.display = 'none';
  document.getElementById('users-panel-overlay').style.display = 'block';
  document.getElementById('users-panel').style.right = '0';
  renderUsersList();
}

function closeUsersPanel() {
  document.getElementById('users-panel-overlay').style.display = 'none';
  document.getElementById('users-panel').style.right = '-540px';
}

function renderUsersList() {
  var users = authGetUsers();
  var grid = document.getElementById('users-panel-list');
  var rolColors = {
    'Admin':           { bg: '#eff6ff', color: '#2563eb', av: '#2563eb' },
    'Monitor':         { bg: '#f0fdf4', color: '#16a34a', av: '#16a34a' },
    'Monitor/Técnico': { bg: '#f5f3ff', color: '#7c3aed', av: '#7c3aed' },
    'Técnico':         { bg: '#fffbeb', color: '#d97706', av: '#d97706' }
  };
  var canEdit = authCan('usuarios');
  grid.innerHTML = users.map(function(u) {
    var rc = rolColors[u.rol] || rolColors['Monitor'];
    var initials = u.nombre.split(' ').map(function(w){ return w[0]; }).slice(0,2).join('').toUpperCase();
    var isSelf = currentUser && currentUser.id === u.id;
    return '<div style="background:var(--white);border:1.5px solid var(--border);border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:12px">' +
      '<div style="width:38px;height:38px;border-radius:50%;background:'+rc.av+';display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#fff;flex-shrink:0">'+initials+'</div>' +
      '<div style="flex:1;min-width:0">' +
        '<div style="font-size:14px;font-weight:600;color:var(--text)">'+escHtml(u.nombre)+(isSelf?' <span style="font-size:10px;color:var(--text3);font-weight:400">(tú)</span>':'')+'</div>' +
        '<div style="font-size:12px;color:var(--text3)">@'+escHtml(u.username)+'</div>' +
      '</div>' +
      '<span style="font-size:11px;padding:3px 10px;border-radius:10px;font-weight:600;background:'+rc.bg+';color:'+rc.color+';margin-right:6px">'+escHtml(u.rol)+'</span>' +
      (canEdit ?
        '<div style="display:flex;gap:5px">' +
          '<button onclick="openEditUser('+u.id+')" title="Editar" style="width:30px;height:30px;border-radius:7px;border:1.5px solid var(--border2);background:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text2)">' +
            '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>' +
          '</button>' +
          (!isSelf ?
            '<button onclick="deleteUser('+u.id+')" title="Eliminar" style="width:30px;height:30px;border-radius:7px;border:1.5px solid var(--border2);background:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text3)">' +
              '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>' +
            '</button>'
          : '') +
        '</div>'
      : '') +
    '</div>';
  }).join('');
}

function deleteUser(id) {
  if (!confirm('¿Eliminar este usuario?')) return;
  var users = authGetUsers().filter(function(u) { return u.id !== id; });
  authSaveUsers(users);
  renderUsersList();
}

// ── MODAL CREAR / EDITAR USUARIO ──
var authEditingId = null;

function openNewUser() {
  authEditingId = null;
  document.getElementById('umodal-title').textContent = 'Nuevo usuario';
  document.getElementById('umodal-nombre').value = '';
  document.getElementById('umodal-username').value = '';
  document.getElementById('umodal-password').value = '';
  document.getElementById('umodal-rol').value = 'Monitor';
  document.getElementById('umodal-pass-hint').style.display = 'block';
  document.getElementById('umodal-err').textContent = '';
  document.getElementById('umodal-bg').style.display = 'flex';
}

function openEditUser(id) {
  var users = authGetUsers();
  var u = users.find(function(x) { return x.id === id; });
  if (!u) return;
  authEditingId = id;
  document.getElementById('umodal-title').textContent = 'Editar usuario';
  document.getElementById('umodal-nombre').value = u.nombre;
  document.getElementById('umodal-username').value = u.username;
  document.getElementById('umodal-password').value = '';
  document.getElementById('umodal-rol').value = u.rol;
  document.getElementById('umodal-pass-hint').style.display = 'block';
  document.getElementById('umodal-err').textContent = '';
  document.getElementById('umodal-bg').style.display = 'flex';
}

function closeUserModal() {
  document.getElementById('umodal-bg').style.display = 'none';
  authEditingId = null;
}

function saveUserModal() {
  var nombre   = document.getElementById('umodal-nombre').value.trim();
  var username = document.getElementById('umodal-username').value.trim();
  var pass     = document.getElementById('umodal-password').value;
  var rol      = document.getElementById('umodal-rol').value;
  var errEl    = document.getElementById('umodal-err');

  if (!nombre || !username) { errEl.textContent = 'Nombre y usuario son obligatorios.'; return; }
  if (!authEditingId && pass.length < 6) { errEl.textContent = 'La contraseña debe tener mínimo 6 caracteres.'; return; }
  if (authEditingId && pass.length > 0 && pass.length < 6) { errEl.textContent = 'Si cambias la contraseña, mínimo 6 caracteres.'; return; }

  var users = authGetUsers();
  var dup = users.find(function(u) { return u.username === username && u.id !== authEditingId; });
  if (dup) { errEl.textContent = 'Ese nombre de usuario ya existe.'; return; }

  if (authEditingId) {
    users = users.map(function(u) {
      if (u.id !== authEditingId) return u;
      return { id: u.id, nombre: nombre, username: username, password: pass.length >= 6 ? pass : u.password, rol: rol };
    });
    // Actualizar sesión si es el usuario actual
    if (currentUser && currentUser.id === authEditingId) {
      currentUser = users.find(function(u) { return u.id === authEditingId; });
      sessionStorage.setItem(AUTH_SESSION_KEY, JSON.stringify(currentUser));
      updateTopbarUser();
    }
  } else {
    users.push({ id: Date.now(), nombre: nombre, username: username, password: pass, rol: rol });
  }

  authSaveUsers(users);
  closeUserModal();
  renderUsersList();
}

// ── INYECTAR LOGIN SCREEN ──
function injectLoginScreen() {
  var style = document.createElement('style');
  style.textContent = [
    '#auth-screen{display:none;position:fixed;inset:0;z-index:1000;background:linear-gradient(135deg,#1e2d5a 0%,#162347 60%,#0f1a38 100%);align-items:center;justify-content:center;flex-direction:column;gap:0}',
    '#auth-card{background:#fff;border-radius:20px;padding:2.5rem 2rem 2rem;width:100%;max-width:360px;display:flex;flex-direction:column;align-items:center;gap:1.25rem;box-shadow:0 24px 64px rgba(0,0,0,.35)}',
    '#auth-screen .auth-logo{width:72px;height:72px;border-radius:50%;background:#2563eb;display:flex;align-items:center;justify-content:center}',
    '#auth-screen .auth-logo svg{width:36px;height:36px;color:#fff;stroke:#fff}',
    '#auth-screen .auth-title{font-size:22px;font-weight:700;color:#111827;text-align:center;font-family:Inter,sans-serif;line-height:1.2}',
    '#auth-screen .auth-sub{font-size:12px;color:#6b7280;text-align:center;font-family:Inter,sans-serif;margin-top:-0.75rem}',
    '#auth-screen .auth-fields{width:100%;display:flex;flex-direction:column;gap:10px}',
    '#auth-screen .auth-fields label{font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;display:block;margin-bottom:3px;font-family:Inter,sans-serif}',
    '#auth-screen .auth-fields input{width:100%;border:1.5px solid #e5e7eb;border-radius:8px;padding:10px 13px;font-size:14px;font-family:Inter,sans-serif;color:#111827;outline:none;transition:border .2s}',
    '#auth-screen .auth-fields input:focus{border-color:#2563eb;box-shadow:0 0 0 3px rgba(37,99,235,.1)}',
    '#auth-screen .auth-btn{width:100%;padding:11px;background:#2563eb;color:#fff;border:none;border-radius:9px;font-size:14px;font-weight:700;font-family:Inter,sans-serif;cursor:pointer;transition:background .2s}',
    '#auth-screen .auth-btn:hover{background:#1d4ed8}',
    '#auth-err{font-size:12px;color:#dc2626;text-align:center;min-height:16px;font-family:Inter,sans-serif}',
    '#auth-screen .auth-hint{font-size:11px;color:#9ca3af;font-family:Inter,sans-serif}',
    '@keyframes authShake{0%,100%{transform:translateX(0)}20%,60%{transform:translateX(-8px)}40%,80%{transform:translateX(8px)}}',
    '#users-panel-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:200;backdrop-filter:blur(2px)}',
    '#users-panel{position:fixed;top:0;right:-540px;width:520px;height:100vh;background:var(--white);border-left:1px solid var(--border);z-index:201;overflow-y:auto;transition:right .28s cubic-bezier(.4,0,.2,1);box-shadow:-4px 0 24px rgba(0,0,0,.08);display:flex;flex-direction:column}',
    '#users-panel-head{padding:18px 24px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;background:var(--white);position:sticky;top:0;z-index:1}',
    '#users-panel-body{padding:20px;display:flex;flex-direction:column;gap:10px;flex:1}',
    '#umodal-bg{display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:400;align-items:center;justify-content:center}',
    '#umodal{background:#fff;border-radius:16px;padding:1.5rem;width:100%;max-width:340px;box-shadow:0 20px 60px rgba(0,0,0,.2)}',
    '#umodal h3{font-size:15px;font-weight:700;color:#111827;margin-bottom:1rem;font-family:Inter,sans-serif}',
    '#umodal .um-row{margin-bottom:10px}',
    '#umodal .um-row label{display:block;font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px;font-family:Inter,sans-serif}',
    '#umodal .um-row input,#umodal .um-row select{width:100%;border:1.5px solid #e5e7eb;border-radius:8px;padding:9px 12px;font-size:13px;font-family:Inter,sans-serif;color:#111827;outline:none}',
    '#umodal .um-row input:focus,#umodal .um-row select:focus{border-color:#2563eb}',
    '#umodal-err{font-size:11px;color:#dc2626;min-height:14px;margin-top:6px;font-family:Inter,sans-serif}',
    '#umodal .um-actions{display:flex;gap:8px;margin-top:1.25rem;justify-content:flex-end}',
    '#umodal .um-cancel{padding:8px 16px;border:1.5px solid #e5e7eb;border-radius:8px;background:none;font-size:13px;cursor:pointer;color:#6b7280;font-family:Inter,sans-serif}',
    '#umodal .um-save{padding:8px 16px;background:#2563eb;color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;font-family:Inter,sans-serif}'
  ].join('');
  document.head.appendChild(style);

  var el = document.createElement('div');
  el.id = 'auth-screen';
  el.innerHTML =
    '<div id="auth-card">' +
      '<div class="auth-logo">' +
        '<svg viewBox="0 0 24 24" fill="none" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>' +
      '</div>' +
      '<div class="auth-title">Monitoreo C.E</div>' +
      '<div class="auth-sub">Sistema de gestión de incidencias</div>' +
      '<div class="auth-fields">' +
        '<div><label>Usuario</label><input type="text" id="auth-username" placeholder="usuario" autocomplete="off" /></div>' +
        '<div><label>Contraseña</label><input type="password" id="auth-password" placeholder="••••••••" /></div>' +
      '</div>' +
      '<div id="auth-err"></div>' +
      '<button class="auth-btn" onclick="doLogin()">Ingresar</button>' +
      '<span class="auth-hint">Admin por defecto: admin / admin123</span>' +
    '</div>';
  document.body.appendChild(el);

  document.addEventListener('keydown', function(e) {
    if (e.key === 'Enter' && document.getElementById('auth-screen').style.display !== 'none') {
      doLogin();
    }
  });
}

// ── INYECTAR PANEL USUARIOS ──
function injectUserModal() {
  // Panel lateral usuarios
  var overlay = document.createElement('div');
  overlay.id = 'users-panel-overlay';
  overlay.onclick = closeUsersPanel;
  document.body.appendChild(overlay);

  var panel = document.createElement('div');
  panel.id = 'users-panel';
  panel.innerHTML =
    '<div id="users-panel-head">' +
      '<h2 style="font-size:16px;font-weight:700">Gestión de usuarios</h2>' +
      '<button onclick="closeUsersPanel()" style="background:none;border:none;cursor:pointer;font-size:13px;color:var(--text2);padding:6px 12px;border-radius:7px;border:1.5px solid var(--border)">✕ Cerrar</button>' +
    '</div>' +
    '<div id="users-panel-body">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">' +
        '<span style="font-size:13px;color:var(--text2)">Usuarios registrados</span>' +
        '<button id="btn-new-user" onclick="openNewUser()" style="display:flex;align-items:center;gap:6px;padding:7px 14px;background:var(--accent);color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer">' +
          '+ Nuevo usuario' +
        '</button>' +
      '</div>' +
      '<div id="users-panel-list" style="display:flex;flex-direction:column;gap:8px"></div>' +
    '</div>';
  document.body.appendChild(panel);

  // Modal crear/editar
  var modal = document.createElement('div');
  modal.id = 'umodal-bg';
  modal.innerHTML =
    '<div id="umodal">' +
      '<h3 id="umodal-title">Nuevo usuario</h3>' +
      '<div class="um-row"><label>Nombre completo</label><input type="text" id="umodal-nombre" placeholder="Ej. Carlos López" /></div>' +
      '<div class="um-row"><label>Usuario (login)</label><input type="text" id="umodal-username" placeholder="Ej. clopez" autocomplete="off" /></div>' +
      '<div class="um-row">' +
        '<label>Contraseña</label>' +
        '<input type="password" id="umodal-password" placeholder="Mínimo 6 caracteres" />' +
        '<div id="umodal-pass-hint" style="font-size:11px;color:#9ca3af;margin-top:3px;font-family:Inter,sans-serif">Al editar, deja en blanco para no cambiar.</div>' +
      '</div>' +
      '<div class="um-row"><label>Rol</label>' +
        '<select id="umodal-rol">' +
          '<option value="Monitor">Monitor</option>' +
          '<option value="Monitor/Técnico">Monitor/Técnico</option>' +
          '<option value="Técnico">Técnico</option>' +
          '<option value="Admin">Admin</option>' +
        '</select>' +
      '</div>' +
      '<div id="umodal-err"></div>' +
      '<div class="um-actions">' +
        '<button class="um-cancel" onclick="closeUserModal()">Cancelar</button>' +
        '<button class="um-save" onclick="saveUserModal()">Guardar</button>' +
      '</div>' +
    '</div>';
  document.body.appendChild(modal);
}

// ── HELPER ESCAPE HTML ──
function escHtml(s) {
  var d = document.createElement('div');
  d.textContent = String(s);
  return d.innerHTML;
}

// ── GUARDIA: bloquear acciones por permisos ──
// Envuelve crearTicket para verificar permiso
var _origCrearTicket;
window.addEventListener('load', function() {
  if (typeof crearTicket === 'function') {
    _origCrearTicket = crearTicket;
    crearTicket = function() {
      if (!authCan('crear')) { showToast('No tienes permiso para crear tickets.', 'err'); return; }
      _origCrearTicket();
    };
  }
  if (typeof cerrarTicket === 'function') {
    var _origCerrar = cerrarTicket;
    cerrarTicket = function() {
      if (!authCan('cerrar')) { showToast('No tienes permiso para cerrar tickets.', 'err'); return; }
      _origCerrar();
    };
  }
  if (typeof guardarEdit === 'function') {
    var _origEdit = guardarEdit;
    guardarEdit = function() {
      if (!authCan('editar')) { showToast('No tienes permiso para editar tickets.', 'err'); return; }
      _origEdit();
    };
  }
  // Iniciar auth después de que todo esté listo
  authInit();
});
